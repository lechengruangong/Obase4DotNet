...
public static class CoreModelRegister
{
    ...
    public static void Regist(ModelBuilder modelBuilder)
    {
        ...
        var idConstructor = idEntityConfiguration.HasConstructor(typeof(Identity).GetConstructor(
            ...
            [typeof(Guid), typeof(DateTime), typeof(string), typeof(DateTime), typeof(string)], null));
        ...
        idConstructor.HasParameter(p => p.Id, typeof(Guid), true)
            ...
            .HasParameter(p => new JsonSerializer().Serialize(p.Name), typeof(string), true, json => new JsonSerializer().Deserialize(json?.ToString(), typeof(string)));
        //Identity没有引用 无需配置
        //忽略版本和次版本
