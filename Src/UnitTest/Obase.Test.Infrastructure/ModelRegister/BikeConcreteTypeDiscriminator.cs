...
public class BikeConcreteTypeDiscriminator : IConcreteTypeDiscriminator
{
    ...
    public BikeConcreteTypeDiscriminator(Type contextType)
    {
    ...
    public StructuralType Discriminate(object typeCode)
    {
        ...
        var bikeType = GlobalModelCache.Current.GetModel(_contextType).GetStructuralType(typeof(Bike));
        var myBikeAType = GlobalModelCache.Current.GetModel(_contextType).GetStructuralType(typeof(MyBikeA));
        var myBikeBType = GlobalModelCache.Current.GetModel(_contextType).GetStructuralType(typeof(MyBikeB));
        ...
        throw new ArgumentException($"未知的Bike类型判别参数值{typeCode}.");
    }
}