...
public class MultiTenantNoDefAnnotationTest
{
    ...
    public void SetUp()
    {
        ...
        foreach (var dataSource in TestCaseSourceConfigurationManager.DataSources)
        {
            ...
            //销毁所有可能的冗余对象
            context.CreateSet<MultiTenantSchoolNoDefAnnotation>().Delete(p => p.SchoolId > 0);
            context.CreateSet<MultiTenantTeacherNoDefAnnotation>().Delete(p => p.TeacherId > 0);
            //加入新对象 保存的对象都是第0个用户的
            ...
            var teacher = new MultiTenantTeacherNoDefAnnotation { School = school, Name = "某老师" };
    ...
    public void QueryTest(EDataSource dataSource)
    {
        ...
        var teacher = context.CreateSet<MultiTenantTeacherNoDefAnnotation>().Include(p => p.School).FirstOrDefault();
        //校验取出的值
        ...
        //此时查询的是第1个用户的值
        teacher = context.CreateSet<MultiTenantTeacherNoDefAnnotation>().Include(p => p.School).FirstOrDefault();
        //无法查出
