...
/// </summary>
[TestFixture]
public class LogicDeletionTest
{
    /// <summary>
    ///     构造实例 为上下文赋值
    /// </summary>
    ...
    public void SetUp()
    {
        foreach (var dataSource in TestCaseSourceConfigurationManager.DataSources)
        {
            ...
            //销毁可能的冗余数据
            context.CreateSet<Obase.AddonTest.Domain.LogicDeletion.LogicDeletion>().Delete(p => p.IntNumber > 0);

            //添加新对象 一半删除一半没删除
            for (var i = 1; i < 21; i++)
                context.Attach(new Obase.AddonTest.Domain.LogicDeletion.LogicDeletion
                {
                    Bool = i % 2 == 0,
    ...
    public void QueryTest(EDataSource dataSource)
    {
        ...
        foreach (var logicDeletion in list)
            ...
            if (logicDeletion.String.Contains("1"))
                context.CreateSet<Obase.AddonTest.Domain.LogicDeletion.LogicDeletion>().RemoveLogically(logicDeletion);
        context.SaveChanges();

        //查询
        list = context.CreateSet<Obase.AddonTest.Domain.LogicDeletion.LogicDeletion>().ToList();

        //有4个
        Assert.That(list.Count, Is.EqualTo(4));

        //测试直接逻辑删除
        context.CreateSet<Obase.AddonTest.Domain.LogicDeletion.LogicDeletion>().DeleteLogically(p => p.IntNumber <= 5);
        ...
        //测试恢复
        context.CreateSet<Obase.AddonTest.Domain.LogicDeletion.LogicDeletion>()
            .RecoveryLogically(p => p.IntNumber >= 0);
