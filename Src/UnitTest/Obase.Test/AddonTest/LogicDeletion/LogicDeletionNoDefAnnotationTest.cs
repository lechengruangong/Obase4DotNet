...
/// </summary>
[TestFixture]
public class LogicDeletionNoDefAnnotationTest
{
    /// <summary>
    ///     构造实例 为上下文赋值
    /// </summary>
    ...
    public void SetUp()
    {
        foreach (var dataSource in TestCaseSourceConfigurationManager.DataSources)
        {
            ...
            //销毁可能的冗余数据
            context.CreateSet<LogicDeletionNoDefAnnotation>().Delete(p => p.IntNumber > 0);

            //添加新对象 全部没删除
            for (var i = 1; i < 21; i++)
                context.Attach(new LogicDeletionNoDefAnnotation
                {
    ...
    public void QueryTest(EDataSource dataSource)
    {
        ...
        foreach (var logicDeletion in list)
            ...
            if (logicDeletion.String.Contains("1"))
                context.CreateSet<LogicDeletionNoDefAnnotation>().RemoveLogically(logicDeletion);
        context.SaveChanges();

        //查询
        list = context.CreateSet<LogicDeletionNoDefAnnotation>().ToList();

        //有9个
        Assert.That(list.Count, Is.EqualTo(9));

        //测试直接逻辑删除
        context.CreateSet<LogicDeletionNoDefAnnotation>().DeleteLogically(p => p.IntNumber <= 5);
        ...
        //测试恢复
        context.CreateSet<LogicDeletionNoDefAnnotation>().RecoveryLogically(p => p.IntNumber >= 0);
