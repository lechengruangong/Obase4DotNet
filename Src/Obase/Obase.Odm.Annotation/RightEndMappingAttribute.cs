...
namespace Obase.Odm.Annotation
{
    ...
    public class RightEndMappingAttribute : MemberAttribute
    {
        /// <summary>
        ///     此端的键属性
        /// </summary>
        ...
        public RightEndMappingAttribute(string keyAttribute, string target)
        {
        ...
        {
            throw new InvalidOperationException("隐式关联右端标注不应调用此方法");
        }
            ...
            if (memberInfo is PropertyInfo propertyInfo)
            {
                ...
                if (configurator.Upward() is StructuralTypeConfiguration structuralTypeConfiguration)
                {
                    ...
                    if (implicitAssociationConfig != null)
                    {
                        ...
                        var rightEnd = ends.FirstOrDefault(p => p.EntityType == type);
                        if (rightEnd != null)
                            rightEnd.HasMapping(_keyAttribute, _target);
                    }
                }
            }
        }
