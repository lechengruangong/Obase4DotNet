...
namespace Obase.Core.Odm
{
    ...
    internal class
        DelegateStructStackValueSetter<TObject, TElement> : DelegateEnumerableStructValueSetter<TObject,
        Stack<TElement>, TElement>
        where TObject : class
    {
        /// <summary>
        ///     创建DelegateStructStackValueSetter实例。
        /// </summary>
        /// <param name="delegate">为元素设值的委托。</param>
        public DelegateStructStackValueSetter(Action<TObject, Stack<TElement>> @delegate) : base(@delegate,
            value => new Stack<TElement>(value))
        {
        }
    }
