...
namespace Obase.Core.Odm
{
    ...
    internal class
        DelegateStructQueueValueSetter<TObject, TElement> : DelegateEnumerableStructValueSetter<TObject,
        Queue<TElement>, TElement>
        where TObject : class
    {
        /// <summary>
        ///     创建DelegateStructQueueValueSetter实例。
        /// </summary>
        /// <param name="delegate">为元素设值的委托。</param>
        public DelegateStructQueueValueSetter(Action<TObject, Queue<TElement>> @delegate) : base(@delegate,
            value => new Queue<TElement>(value))
        {
        }
    }
