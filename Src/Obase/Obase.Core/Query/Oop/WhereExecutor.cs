...
namespace Obase.Core.Query.Oop
{
    ...
    /// </summary>
    public class WhereExecutor<TSource> : OopExecutor
    {
        /// <summary>
        ///     要执行的查询运算
        /// </summary>
        private readonly WhereOp _op;
        ...
        public override void Execute(OopContext context)
        {
            //执行Where
            context.Result =
                ((IEnumerable<TSource>)context.Result).Where(
                    (Func<TSource, bool>)ExpressionDelegates.Current[_op.Predicate]);
            //调用管道下一节处理
